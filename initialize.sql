...
CREATE TABLE PESSOA(
    ...
    CONSTRAINT CK_CPF CHECK(REGEXP_LIKE(CPF, '[0-9]{3}\.[0-9]{3}\.[0-9]{3}\-[0-9]{2}')), -- CHECAR SE ISSO ESTA CERTO

);

CREATE TABLE CLIENTE(
    --CHECAR SE PODE MESMO SER VAZIO
)INHERITS(PESSOA);
...
CREATE TABLE ALOCACAO(
    ...
    CONSTRAINT PK_ALOCACAO      PRIMARY KEY(CPF_FUNCIONARIO, NRO_BALCAO, DIA, HORA)      ON DELETE CASCADE, -- CHECAR SE EH CASCADE MESMO
    CONSTRAINT FK_NRO_BALCAO    FOREIGN KEY(NRO_BALCAO) REFERENCES BAR(NRO_BALCAO)       ON DELETE SET NULL,
    CONSTRAINT FK_DATA          FOREIGN KEY(DIA, HORA)  REFERENCES DATA_ALOCAO(DIA,HORA) ON DELETE SET NULL, -- CHECAR SET NULL
...
CREATE TABLE FANTASIA(
    CODIGO_BARRAS   CHAR(13)    PRIMARY KEY NOT NULL,
    NOME            VARCHAR(20),
    COR             VARCHAR(10),
    TAMANHO         CHAR(1),
...
CREATE TABLE FESTA(
    ...
    CONSTRAINT PK_FESTA          PRIMARY KEY(DIA, HORA) ON DELETE CASCADE, -- CHECAR SE EH DELETE CASCADE MESMO
    CONSTRAINT CK_TIPO           CHECK(UPPER(TIPO) IN ('CARNAVAL', 'BALADA')),
    -- CHECANDO PARA NAO TER VALORES NEGATIVOS PARA INGRESSO
    CONSTRAINT CK_VALOR_INGRESSO CHECK(VALOR_INGRESSO => 0),
);

CREATE TABLE CARNAVAL(
    -- CHECAR SE PODE FICAR VAZIO MESMO
)INHERITS(FESTA);
...
CREATE TABLE COMANDA(
    ...
    CODIGO_BARRAS   CHAR(13)                    DEFAULT '0', -- CHECAR SE PODE SER NULL CASO NAO TENHA PEGO FANTASIA
    ...
    CONSTRAINT FK_CODIGO_BARRAS FOREIGN KEY(CODIGO_BARRAS)          REFERENCES FANTASIA(CODIGO_BARRAS)  ON DELETE SET NULL,
...
CREATE TABLE ALUGUEL(
    ...
    CONSTRAINT PK_ALUGUEL       PRIMARY KEY(CODIGO, CODIGO_BARRAS) ON DELETE CASCADE,
    CONSTRAINT FK_CODIGO_BARRAS FOREIGN KEY(CODIGO_BARRAS)         REFERENCES FANTASIA(CODIGO_BARRAS)  ON DELETE SET NULL, -- CHECAR OS SET NULL E CASCADE
...
CREATE TABLE CONTRATO_BANDA(
    ...
    BANDA           VARCHAR(20)    NOT NULL,
    CARNAVAL        DATE           NOT NULL,                    -- CHECAR SE EH SO O DIA OU A HORA
    VALOR_PAGO      REAL                        DEFAULT '0',

    CONSTRAINT  PK_CONTRATO_BANDA   PRIMARY KEY(NRO_CONTRATO, BANDA, DATA, CARNAVAL),
    CONSTRAINT  FK_BANDA            FOREIGN KEY(BANDA)      REFERENCES BANDA(NOME)          ON DELETE CASCADE, -- CHECAR CASCADE
    CONSTRAINT  FK_CARNAVAL         FOREIGN KEY(CARNAVAL)   REFERENCES CARNAVAL(DIA,HORA)   ON DELETE CASCADE, -- CHECAR CASCADE E ARRUMAR DIA HORA
...
CREATE TABLE CONTRATO_DJ(
    NRO_CONTRATO    INT         NOT NULL,
    BALADA          DATE        NOT NULL,                   --CHECAR SE PODE COLOCAR COM DATA
    DJ              VARCHAR(20) NOT NULL,
    VALOR_PAGO      REAL                    DEFAULT '0',

    CONSTRAINT  PK_CONTRATO_DJ PRIMARY KEY(NRO_CONTRATO, BALADA, DJ),
    CONSTRAINT  FK_BALADA      FOREIGN KEY(BALADA)  REFERENCES BALADA(DIA, HORA) ON DELETE CASCADE, -- CHECAR DELETE CASCADE E DIA HORA_FESTA
    CONSTRAINT  FK_DJ          FOREIGN KEY(DJ)      REFERENCES DJ(NOME)          ON DELETE CASCADE, -- CHECAR CASCADE
...
CREATE TABLE BEBIDA(
    ...
    CONSTRAINT PK_BEBIDA     PRIMARY KEY(NRO_BALCAO, CODIGO_BARRAS)                 ON DELETE CASCADE, -- CHECAR SE E CASCADE MESMO
    CONSTRAINT FK_NRO_BALCAO FOREIGN KEY(NRO_BALCAO)    REFERENCES BAR(NRO_BALCAO)  ON DELETE SET NULL,
